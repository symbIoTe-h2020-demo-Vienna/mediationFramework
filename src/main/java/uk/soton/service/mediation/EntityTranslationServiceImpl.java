...
/**
 * The Class EntityTranslationServiceImpl implements the EntityTranslationService interface using Jena API as an RDF
 * framework.
 *
...
public class EntityTranslationServiceImpl implements EntityTranslationService {
    ...
    public Triple instantiatePattern(Triple t,
        ...
        if (binding.containsKey(predicate)) {
        ...
        if (subject.isBlank()) {
            subject = Utility.getVarFromBlank(subject);
        }
        if (object.isBlank()) {
            object = Utility.getVarFromBlank(object);
        }
    ...
    /**
     * The instanciateBindings method create the data for generate the OpAssign operations for assign correct values
     * when unbounded vars appears
     *
