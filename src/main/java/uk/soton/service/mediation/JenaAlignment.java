...
/**
 * The Class JenaAlignment implements the Alignment interface using Jena API. At class loading time the resolver is
 * populated with well known functions instances.
 *
...
public class JenaAlignment implements Alignment {
    ...
    private static Node getNewVar() {
        return NodeFactory.createVariable("_" + getNewSymb());
    }
    /**
...

    {
        ...
        resolver.put(NodeFactory.createURI(RDFVocabulary.SAMEAS), FunctionWrapper.$(new SameAs()));
        resolver.put(NodeFactory.createURI(RDFVocabulary.FN_SUB), FunctionWrapper.$(new E_Subtract(null, null)));
        resolver.put(NodeFactory.createURI(RDFVocabulary.FN_SUM), FunctionWrapper.$(new E_Add(null, null)));
     ...
     *
     * @param m the RDF model to use to build the alignment
     */
    ...
    private Resource getA(BlankNodeId id) {
        return (id == null
                ? this.inner.createResource(AnonId.create())
                : this.inner.createResource(id.getLabelString()));
    }
    ...
    public List<String> getSourceOntologyURIs() {
        ...
        ExtendedIterator<Triple> source = this.inner.getGraph().find(null,
                NodeFactory.createURI(RDFVocabulary.HAS_SOURCE_ONTOLOGY), null);
    ...
    public List<String> getTargetOntologyURIs() {
        ArrayList<String> result = new ArrayList<String>();
        ExtendedIterator<Triple> source = this.inner.getGraph().find(null,
                NodeFactory.createURI(RDFVocabulary.HAS_TARGET_ONTOLOGY), null);
        for (Triple t : source.toList()) {
            result.add(t.getObject().getURI());
        }
        return result;
    ...
    public void addPattern(Triple lhs, List<Triple> rhs, Relation relation) {
        ...
        ReifierStd.reifyAs(this.inner.getGraph(), lhsa.asNode(), lhs);
        for (Triple t : rhs) {
            Resource rhsa = this.getA();
            this.inner.add(ea, this.getP(RDFVocabulary.RHS), rhsa);
            ReifierStd.reifyAs(this.inner.getGraph(), rhsa.asNode(), t);
        }
    ...
    public void addRewritingRule(RewritingRule rule) {
        Resource lhsa = this.getA();
        Resource ea = this.getA();
        ReifierStd.reifyAs(this.inner.getGraph(), lhsa.asNode(), rule.getLHS());
        this.inner.add(ea, this.getP(RDFVocabulary.RDF_TYPE),
        ...
        for (FunctionalDependency fd : rule.getFD()) {
            ...
            ReifierStd.reifyAs(this.inner.getGraph(), fdr.asNode(), new Triple(fd.getVar(), this.getP(fd.getFuncURI())
     ...
     *
     * @param t Triple that represents a functional dependency
     ...
     * Gets the collection.
     *
     * @param root Root node of the RDF collection
    ...
    private List<Node> getCollection(Node root) {
        ...
        List<Triple> fi = this.inner.getGraph()
                .find(root, NodeFactory.createURI(RDFVocabulary.FIRST), null).toList();
        if (!fi.isEmpty()) {
            ...
            return result;
                ...
                .find(root, NodeFactory.createURI(RDFVocabulary.SECOND), null)
                .toList();
        if (!fi.isEmpty()) {
            Node par = fi.get(0).getMatchObject();
            if (par.isBlank()) {
            ...
            return result;
                ...
                .find(root, NodeFactory.createURI(RDFVocabulary.THIRD), null).toList();
        if (!fi.isEmpty()) {
            Node par = fi.get(0).getMatchObject();
            if (par.isBlank()) {
                par = Utility.getVarFromBlank(par);
