...
public class ClassVisitor implements EDOALVisitor, MediationResultGenerator {
 ...
 /*@Override
    ...
    private void innerVisit(Object ce) throws AlignmentException {
        if (ce instanceof ClassId) {
            this.visit((ClassId) ce);
        } else if (ce instanceof ClassConstruction) {
            this.visit((ClassConstruction) ce);
        } else if (ce instanceof ClassValueRestriction) {
            this.visit((ClassValueRestriction) ce);
        } else if (ce instanceof ClassDomainRestriction) {
            this.visit((ClassDomainRestriction) ce);
        } else if (ce instanceof ClassOccurenceRestriction) {
            this.visit((ClassOccurenceRestriction) ce);
        } else if (ce instanceof ClassTypeRestriction) {
            this.visit((ClassTypeRestriction) ce);
        } else {
            Logger.getAnonymousLogger().log(Level.WARNING, "Class not handled yet:" + ce.getClass());
        }
    }
    ...
    public void visit(ClassValueRestriction cvr) throws AlignmentException {
        if (cvr.getComparator() == Comparator.EQUAL) {
            ...
            Logger.getAnonymousLogger().log(Level.WARNING, "Only EQUAL restriction allowed.");
        }
    }
    ...
    public void visit(ClassId ce) {
        ...
        this.mr.getPatterns().add(new STriple(cs, RDF.type.asNode(), NodeFactory.createURI(ce.getURI().toString())));
    }
    ...
    public void visit(ClassConstruction ce) throws AlignmentException {
        if (ce.getOperator() == SyntaxElement.Constructor.AND) {
            ...
            Logger.getAnonymousLogger().log(Level.WARNING, "Only AND constructor supported.");
        }
    }
