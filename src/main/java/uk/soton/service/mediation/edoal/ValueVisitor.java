...
public class ValueVisitor implements EDOALVisitor, ValueGenerator {
    ...
    private void innerVisit(Object v) throws AlignmentException {
        if (v instanceof InstanceId) {
            visit((InstanceId) v);
        } else if (v instanceof Value) {
            visit((Value) v);
        } else if (v instanceof Apply) {
            visit((Apply) v);
        } else if (v instanceof PropertyId) {
            visit((PropertyId) v);
        } else if (v instanceof RelationId) {
            visit((RelationId) v);
        } else {
            Logger.getAnonymousLogger().log(Level.WARNING, "Value type not supported:" + v);
        }
    }
    ...
    public void visit(Apply v) throws AlignmentException {
        this.value = (this.value == null ? NodeFactory.createBlankNode() : this.value);
    ...
    public void visit(RelationId v) {
        this.value = NodeFactory.createURI(v.getURI().toString());
    }
    ...
    public void visit(Value v) {
        if (v.getType() != null) {
            this.value = NodeFactory.createLiteral(v.getValue(), null, TypeMapper.getInstance().getSafeTypeByName(v.getType().toString()));
        } else {
            this.value = NodeFactory.createLiteral(v.getValue());
        }
    }
