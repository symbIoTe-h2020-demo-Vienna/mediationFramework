...
public class Utility {
     ...
     * @return <b>null</b> if the two Triples are not matchable, otherwise it returns an Hashtable with the resulting
     * bindings otherwise
     */
     ...
     * @return <b>null</b> if the two Nodes are not matchable, otherwise an Hashtable with the resulting bindings
     * otherwise
     */
    ...
    public static Node getVarFromBlank(Node var) {
        return NodeFactory.createVariable(var.getBlankNodeLabel().replaceAll("\\p{Punct}", ""));
    }
    ...
    public static Triple getReified(Node n, Graph g) {
        try {
            Triple result = new Triple(
                    g.find(n, NodeFactory.createURI("http://www.w3.org/1999/02/22-rdf-syntax-ns#subject"), null).next().getMatchObject(),
                    g.find(n, NodeFactory.createURI("http://www.w3.org/1999/02/22-rdf-syntax-ns#predicate"), null).next().getMatchObject(),
                    g.find(n, NodeFactory.createURI("http://www.w3.org/1999/02/22-rdf-syntax-ns#object"), null).next().getMatchObject());
            return result;
        } catch (Exception e) {
